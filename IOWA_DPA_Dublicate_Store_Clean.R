...
library(geonames)
library(ggmap)
register_google(key = "AIzaSyCpf1z_Cy6l_bv7aJ80hPr2rka56BIOpGM", write = TRUE)
...
for (i in 1:nrow(blended_master4)) {
  city_google[i]=revgeo(longitude = lon[i], latitude = lat[i],provider = 'google',API = 'AIzaSyCpf1z_Cy6l_bv7aJ80hPr2rka56BIOpGM',  output='frame')[3]
  county_google[i]=revgeo(longitude = lon[i], latitude = lat[i],provider = 'google',API = 'AIzaSyCpf1z_Cy6l_bv7aJ80hPr2rka56BIOpGM',  output='frame')[4]
  zip_google[i]=revgeo(longitude = lon[i], latitude = lat[i],provider = 'google',API = 'AIzaSyCpf1z_Cy6l_bv7aJ80hPr2rka56BIOpGM',  output='frame')[6]
  lat_check[i]=lat[i]
  lon_check[i]=lon[i]
}
